...
def format_start(result: dict) -> str:
    ...
    return f"새 팀 시작: {result['team_id']}"
...
def format_status(data: dict) -> str:
    ...
    if execution_summary:
        lines.append(
            "실행 요약: dispatching={dispatching} waiting_interactions={waiting} blocked={blocked} run_age={run_age}s task_age={task_age}s interaction_age={interaction_age}s last_event={event}".format(
                dispatching=execution_summary.get("dispatching_tasks", 0),
                waiting=execution_summary.get("waiting_interactions", 0),
                blocked=execution_summary.get("blocked_tasks", 0),
                run_age=f"{execution_summary.get('run_age_seconds', 0):.1f}" if execution_summary.get("run_age_seconds") is not None else "-",
                task_age=f"{execution_summary.get('latest_task_update_age_seconds', 0):.1f}" if execution_summary.get("latest_task_update_age_seconds") is not None else "-",
                interaction_age=f"{execution_summary.get('oldest_pending_interaction_age_seconds', 0):.1f}" if execution_summary.get("oldest_pending_interaction_age_seconds") is not None else "-",
    ...
    if agents:
        for agent in agents:
            ...
            lines.append(
                ...
                f"pane={pane} task={task} runtime={runtime_status} pid={agent.get('pid') or '-'} "
                f"exit={agent.get('exit_code') if agent.get('exit_code') is not None else '-'}"
            )
    ...
    if interactions:
        for item in interactions:
            message = item.get("message") or item.get("payload", {}).get("message") or item["kind"]
            lines.append(
    ...
    if runtime:
        lines.append(
            "  - missing_runtime_state={missing} stale_heartbeat={stale} runtime_not_ready={not_ready} exited_runners={exited} uncollected_outputs={uncollected} orphaned_tasks={tasks} dangling_runs={runs}".format(
                missing=len(runtime.get("missing_runtime_state", [])),
                stale=len(runtime.get("stale_heartbeat", [])),
                not_ready=len(runtime.get("runtime_not_ready", [])),
                exited=len(runtime.get("exited_runners", [])),
                uncollected=len(runtime.get("uncollected_outputs", [])),
...
def format_recover(data: dict) -> str:
    ...
    lines = [
        ...
        f"  - runtime_not_ready: {len(data.get('runtime_not_ready', []))}",
        f"  - exited_runners: {len(data.get('exited_runners', []))}",
        f"  - uncollected_outputs: {len(data.get('uncollected_outputs', []))}",
    ...
    if actions:
        ...
        lines.append("  - 없음")
    if data.get("collected_outputs"):
        lines.append(f"회수된 출력: {len(data['collected_outputs'])}")
